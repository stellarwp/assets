...
SCRIPT,
	...
	protected function assert_minified_found( $slug_prefix, $is_js = true, $has_min = true, $has_only_min = false, $id = '', $add_to_path_group = '', $group_path_path = '', $wont_figure_out_min_vs_unmin = false, $should_prefix = true ) {
		...
		$path = wp_normalize_path( WP_PLUGIN_DIR . ( $group_path_path ? $group_path_path : '/assets/tests/_data/' ) . ( $should_prefix ? ( $is_js ? 'js/' : 'css/' ) : '' ) . $slug_prefix . ( $is_js ? '.js' : '.css' ) );
		...
		$this->assertEquals( $path, $asset->get_full_resource_path( false ) );
	}
